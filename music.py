...
# ----------------- Helpers -----------------
def _fetch_track_info(artist: str, title: str):
    """
    Call track.getInfo to enrich data with listeners/playcount and album art.
    """
...
def get_music_recommendation(mood: str, language: str = "Any", limit: int = 5):
    """
    Returns list of tracks with enriched metadata.
    """
    url = "http://ws.audioscrobbler.com/2.0/"
    ...
    for tag in candidates:
        ...
        for t in tracks:
            ...
            info = _fetch_track_info(artist, title)
            listeners = info.get("listeners") or info.get("playcount") or "N/A"
            image_url = _extract_image(info)
